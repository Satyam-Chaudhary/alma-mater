...
const NetworkHubCollege: React.FC<{ collegeName: string }> = ({ collegeName }) => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        const response = await fetch(`/api/networkApi/${collegeName}`);
        if (!response.ok) {
  ...
  return (
    <div className="relative p-6 bg-slate-50 h-3/4 shadow-lg rounded-lg overflow-y-scroll">
      <h2 className="text-2xl font-bold mb-4 ">Job/Referral Posting via Alumni</h2>
      <div className="absolute top-4 right-4">
        <Button variant="primaryBtn">
          <Link href={`/protected/navFunctionForms/networkForm/?college=${collegeName}`}>
            Create Job Posting
          </Link>
        </Button>
      </div>
      <div className="mt-16">
        {events.length > 0 ? (
          ...
          <p>No Jobs from Alumni found for {collegeName}.</p>
        )}
      </div>
    </div>
  );
...
export default NetworkHubCollege;